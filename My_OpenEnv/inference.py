...
ENVIRONMENT VARIABLES:
  HF_TOKEN            - API key (optional; local heuristic if missing)
...
def create_openai_client(config: Dict[str, Any]) -> Optional[Any]:
    """Return an OpenAI client when available, else None for local fallback."""
    if not HAS_OPENAI or not config.get("hf_token"):
        return None
    try:
        return OpenAI(base_url=config["api_base_url"], api_key=config["hf_token"])
    except Exception as e:
        print(f"[WARNING] OpenAI client unavailable; using local heuristic: {e}", file=sys.stderr)
        return None
...
def get_heuristic_action(obs: Dict[str, Any]) -> Tuple[str, str]:
    """Use a local BFS-based heuristic to reach the target while avoiding obstacles."""
    agent_x, agent_y = obs["agent_position"]
    target_x, target_y = obs["target_position"]
    grid_size = obs.get("grid_size", 20)
    obstacles = {tuple(pos) for pos in obs.get("obstacles", [])}
    agent = (agent_x, agent_y)
    target = (target_x, target_y)

    if agent == target:
        return "right 0.34", "right 0.34"

    # BFS to find a safe shortest path to target
    queue = deque([agent])
    came_from = {agent: None}
    ...
    if target in came_from:
        current = target
        while came_from[current] != agent:
            current = came_from[current]
        dx = current[0] - agent_x
        dy = current[1] - agent_y
        if dx > 0:
            direction = "right"
        elif dx < 0:
            direction = "left"
            ...
            direction = "down"
        return f"{direction} 0.34", f"{direction} 0.34"

    # Fallback to distance-based direction if no clear path found
    dx = target_x - agent_x
    dy = target_y - agent_y
    if abs(dx) >= abs(dy):
    ...
    for direction in [primary, secondary, "right", "up", "left", "down"]:
        next_pos = {
...
def get_llm_action(client: Optional[Any], config: Dict[str, Any], step: int, 
                  obs: Dict[str, Any], history: List[str]) -> Tuple[str, Optional[str]]:
    """
    Get action from LLM or local fallback with error handling
    Returns: (action_string, error_message_or_None)
    """
    if client is None:
        action_str, _ = parse_llm_response("", obs)
...
def run_episode(client: Optional[Any], env: StochasticGridEnvironment, config: Dict[str, Any],
    ...
    try:
        ...
        success = any(abs(r - 10.0) < 1e-6 for r in rewards_list)
...
def create_gradio_ui(client: Optional[Any], config: Dict[str, Any]) -> Optional[Any]:
...
def main():
    ...
    # Initialize OpenAI client if available, otherwise use local heuristic
    client = create_openai_client(config)
    if client is None:
        print("[INFO] OpenAI client not configured; using local heuristic agent.", file=sys.stderr)
